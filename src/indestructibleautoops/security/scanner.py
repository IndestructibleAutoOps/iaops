...
class SecurityIssue:
    ...
    source_path: Optional[str] = None         # File/dependency where issue was found
    source_line: Optional[int] = None         # Line number (if applicable)
    
    # Vulnerability details (if applicable)
    cve_id: Optional[str] = None              # CVE identifier
    ...
    package_name: Optional[str] = None        # Vulnerable package name
    package_version: Optional[str] = None     # Current vulnerable version
    fixed_version: Optional[str] = None       # Available fixed version
    
    # Remediation
    remediation: Optional[str] = None         # Recommended remediation steps
    references: List[str] = field(default_factory=list)  # Reference URLs
    
    # Metadata
    discovered_at: datetime = field(default_factory=datetime.utcnow)
    scan_id: Optional[str] = None             # Associated scan ID
...
class SecurityScanResult:
    ...
    def get_blocking_issues(self) -> List[SecurityIssue]:
        ...
        return self.get_issues_by_severity(SecuritySeverity.CRITICAL) + \
               self.get_issues_by_severity(SecuritySeverity.HIGH)
...
class ScannerRegistry:
    ...
    def get(self, scanner_name: str) -> Optional[SecurityScanner]:
